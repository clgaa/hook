...
//    #include <GLES/gl.h>  
  ...
  #include <object.h> 
    ...
    void (*old_eglSwapBuffers)(long dpy, int surf) = -1;  
      
    void new_eglSwapBuffers(long dpy, int surface)  
    {  
        LOGD("New eglSwapBuffers\n");  
        if (old_eglSwapBuffers == -1)  
            LOGD("error\n");  
        return old_eglSwapBuffers(dpy / 4, surface);  
    }  
    ...
    #define LIBSF_PATH  "/system/lib/libjavacore.so"    
    int hook_eglSwapBuffers()    
    {    
        old_eglSwapBuffers = wait;    
